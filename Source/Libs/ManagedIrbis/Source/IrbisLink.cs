...
namespace ManagedIrbis
{
    ...
    public sealed class IrbisLink
    {
        ...
        private static string _GetPositionalParameter
            (
        ...
        private static bool _GetNamedParameter
            (
            ...
            )
        {
            ...
            if (string.IsNullOrEmpty(key) || navigator.ReadChar() != '=')
            {
                return false;
            }
        ...
        private static string _SetPositionalParameter
            (
                [NotNull] Dictionary<string,string> dictionary,
                [NotNull] string key,
                string value
            ...
            )
        {
            ...
            result.Command = _GetPositionalParameter(navigator);
            if (result.Command.SameString("3"))
            {
                result.Path = _GetPositionalParameter(navigator);
                result.Database = _GetPositionalParameter(navigator);
                result.FileName = _GetPositionalParameter(navigator);
            }
            else
            {
                _GetPositionalParameter(navigator);
                result.Database = _GetPositionalParameter(navigator);
                result.Format = _GetPositionalParameter(navigator);
                result.Key = _GetPositionalParameter(navigator);
            }

            if (navigator.ReadChar() == '?')
            {
                while (_GetNamedParameter(navigator, result.Parameters))
                {
                    // Do nothing
                }

                result.Command = _SetPositionalParameter(result.Parameters, "C21COM", result.Command);
                result.Database = _SetPositionalParameter(result.Parameters, "I21DBN", result.Database);
                result.Format = _SetPositionalParameter(result.Parameters, "PFTNAME", result.Format);
                result.Key = _SetPositionalParameter(result.Parameters, "KEY", result.Key);
                result.Path = _SetPositionalParameter(result.Parameters, "PATH", result.Path);
                result.FileName = _SetPositionalParameter(result.Parameters, "FILENAME", result.FileName);
            }
            ...
            )
        {
            ...
            result.Command = "3";
            result.Path = _GetPositionalParameter(navigator);
            result.Database = _GetPositionalParameter(navigator).EmptyToNull();
            result.FileName = _GetPositionalParameter(navigator);
        ...
        {
            return $"Command: {Command.ToVisibleString()}";
        }
