...
//        }
        ...
        [TestMethod]
        public void IniFileLine_Constructor_1()
        {
            IniFile.Line line = new IniFile.Line();
            Assert.AreEqual(line.Key, string.Empty);
            Assert.IsNull(line.Value);
        }

        [TestMethod]
        public void IniFileLine_SaveRestore_1()
        {
            IniFile.Line line1 = new IniFile.Line("Hello", "World");
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream, Encoding.UTF8);
            line1.SaveToStream(writer);
            stream.Position = 0;
            BinaryReader reader = new BinaryReader(stream);
            IniFile.Line line2 = new IniFile.Line();
            line2.RestoreFromStream(reader);
            Assert.AreEqual(line1.Key, line2.Key);
            Assert.AreEqual(line1.Value, line2.Value);
        }

        [TestMethod]
        public void IniFileSection_SaveRestore_1()
        {
            IniFile iniFile1 = new IniFile();
            IniFile.Section section1 = iniFile1.CreateSection("Main");
            section1.SetValue("Hello", "World");
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            section1.SaveToStream(writer);
            stream.Position = 0;
            BinaryReader reader = new BinaryReader(stream);
            IniFile iniFile2 = new IniFile();
            IniFile.Section section2 = iniFile2.CreateSection("Main");
            section2.RestoreFromStream(reader);
            Assert.AreEqual
                (
                    iniFile1.GetValue("Main", "Hello", "?"),
                    iniFile2.GetValue("Main", "Hello", "??")
                );
        }
...
//        }
        ...
        [TestMethod]
        public void IniFile_SaveRestore_1()
        {
            IniFile file1 = new IniFile();
            file1.SetValue("Main", "Hello", "World");
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            file1.SaveToStream(writer);
            stream.Position = 0;
            BinaryReader reader = new BinaryReader(stream);
            IniFile file2 = new IniFile();
            file2.RestoreFromStream(reader);
            Assert.AreEqual
                (
                    file1.GetValue("Main", "Hello", "?"),
                    file2.GetValue("Main", "Hello", "??")
                );
        }
    }
}
